<x-modal wire:model="modal_version" maxWidth="md" label="true" close="resetUI()">
    <x-slot name="title">
    ...
    <x-slot name="content">
        <div class="mt-4">
            ...
            <div class="relative z-0 mb-6 w-full group">
                <input wire:model.defer="file" type="file" name="image" id="files_version" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  focus:outline-none focus:ring-0 focus:border-blue-600 peer" webkitdirectory multiple>
                <label for="files_version" class="required peer-focus:font-medium absolute text-sm text-gray-500  duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Archivos de guaradado</label>
    ...
    <x-slot name="footer">
        <button wire:click.prevent="Store()" type="button" class="mr-0 md:mr-2 mt-2 md:mt-0 text-white bg-blue-700 hover:bg-blue-800 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
            Guardar
        </button>
